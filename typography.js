...
(function (root) {
  ...
  const presets = Object.freeze([
    ...
    { id: 'humanist', name: 'Humanist', description: 'Warm, open letterforms for approachable storytelling.', display: '"Avenir Next", "Trebuchet MS", Arial, sans-serif', body: '"Avenir Next", "Trebuchet MS", Arial, sans-serif', weight: 600, tracking: -0.037, leading: 1.04 }
  ].map(Object.freeze));
  const limits = Object.freeze({ scale: [0.9, 1.15], leading: [0.95, 1.15], tracking: [-0.015, 0.025], spacing: [0.85, 1.2] });
  ...
  function apply(board, options = {}) {
    ...
    const headlineSize = Math.max(48, ({ editorial: 68, impact: 80, story: 60 }[layout]) * headlineTier * settings.scale);
    const bodySize = Math.max(18, (layout === 'impact' ? 19 : 20) * settings.scale);
    const variables = {
      '--type-display': preset.display,
      '--type-body': preset.body,
      '--type-weight': preset.weight,
      ...
      '--type-body-leading': Math.max(1.3, 1.43 * settings.leading).toFixed(3),
      '--type-body-tracking': `${Math.max(-0.005, settings.tracking * 0.3).toFixed(4)}em`,
      '--type-subhead-size': `${Math.max(20, 22 * settings.scale).toFixed(2)}px`,
      '--type-section-gap': `${(20 * settings.spacing).toFixed(2)}px`,
      '--type-heading-gap': `${(19 * settings.spacing).toFixed(2)}px`,
      '--type-column-gap': `${(40 * settings.spacing).toFixed(2)}px`
    };
    Object.entries(variables).forEach(([key, value]) => board.style.setProperty(key, value));
    board.dataset.typography = settings.preset;
    states.set(board, { settings, headlineSize, bodySize });
    return settings;
  }
  ...
  function inspect(board) {
    ...
    return { ok: issues.length === 0, measurable: true, issues, headlineSize: parseFloat(board.style.getPropertyValue('--type-headline-size')) || states.get(board)?.headlineSize, bodySize: states.get(board)?.bodySize };
  }
  ...
  function fit(board, options) {
    ...
    const adjustments = [];
    // Recover only display space. Body copy keeps its user-selected readable size.
    // Excess copy remains visibly flagged rather than silently hidden or reduced.
    if (!result.ok && result.measurable) {
      for (const ratio of [0.94, 0.88, 0.82]) {
        const size = Math.max(48, state.headlineSize * ratio);
        board.style.setProperty('--type-headline-size', `${size.toFixed(2)}px`);
        result = inspect(board);
        if (result.ok || size === 48) {
          adjustments.push(`Headline fitted at ${Math.round(size)} px; body text stays at ${Math.round(state.bodySize)} px.`);
          break;
        }
      }
